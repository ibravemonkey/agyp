...
func TestShouldTriggerAutoUpdate_CommandExclusions(t *testing.T) {
	...
	tmpDir := t.TempDir()
	t.Setenv("AGYP_DIR", tmpDir)
	t.Setenv("AGYP_NO_AUTO_UPDATE", "")
	t.Setenv("AGYP_AUTO_UPDATE", "")
	t.Setenv("CI", "")

	excluded := []string{"__bg-updater", "herdr-hook", "statusline-hook", "completion", "__complete", "upgrade", "update", "help", "version", "alias"}
...
func TestShouldTriggerAutoUpdate_EnvDisables(t *testing.T) {
	...
	// AGYP_NO_AUTO_UPDATE=1
	t.Setenv("AGYP_NO_AUTO_UPDATE", "1")
	t.Setenv("AGYP_AUTO_UPDATE", "")
	t.Setenv("CI", "")
	if ShouldTriggerAutoUpdate("list") {
		t.Errorf("ShouldTriggerAutoUpdate() should be false when AGYP_NO_AUTO_UPDATE=1")
	}

	// AGYP_AUTO_UPDATE=off
	t.Setenv("AGYP_NO_AUTO_UPDATE", "")
	t.Setenv("AGYP_AUTO_UPDATE", "off")
	if ShouldTriggerAutoUpdate("list") {
		t.Errorf("ShouldTriggerAutoUpdate() should be false when AGYP_AUTO_UPDATE=off")
	}
...
func TestShouldTriggerAutoUpdate_DevVersionGuard(t *testing.T) {
	...
	// With AGYP_AUTO_UPDATE_DEV=1 override
	t.Setenv("AGYP_AUTO_UPDATE_DEV", "1")
	if !ShouldTriggerAutoUpdate("status") {
		t.Errorf("ShouldTriggerAutoUpdate() should be true when AGYP_AUTO_UPDATE_DEV=1 is explicitly set")
	}
}
