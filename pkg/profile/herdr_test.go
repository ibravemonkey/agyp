...
func TestReportHerdrQuotaOnly_UpdatesAllPanesOfProfile(t *testing.T) {
	...
	go func() {
		for {
			...
			_ = conn.SetDeadline(time.Now().Add(500 * time.Millisecond))
			if strings.Contains(reqStr, "pane.list") {
				resp := `{"id":"agyp:panes:1","result":{"panes":[{"pane_id":"w1:p1","agent":"Antigravity","display_agent":"multi-prof","title":"agyp: Task 1","tokens":{"profile":"multi-prof","model":"gemini-2.5-flash","conversation_title":"Task 1","quota_5h_normal":"100%"}},{"pane_id":"w1:p2","agent":"claude","title":"claude code","tokens":{}},{"pane_id":"w1:p3","agent":"Antigravity","display_agent":"multi-prof","title":"agyp: Task 2","tokens":{"profile":"multi-prof","model":"gemini-2.5-flash","conversation_title":"Task 2","quota_5h_normal":"100%"}}]}}`
				_, _ = conn.Write([]byte(resp + "\n"))
			} else {
				_, _ = conn.Write([]byte(`{"id":"agyp:metadata:1","result":"ok"}` + "\n"))
				if strings.Contains(reqStr, "pane.report_metadata") {
	...
	if !ok {
		...
		if !strings.Contains(p3Payload, `"title":"agyp: Task 2"`) {
			t.Errorf("Expected w1:p3 to preserve its own title 'agyp: Task 2', got: %s", p3Payload)
		}
	}
}
