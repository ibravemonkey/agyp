...
func MatchProject(filterProject string, sess ConversationSession) bool {
	...
	// Filter is a plain name (e.g. "agyp", "caudata")
		...
		if len(line) > 0 {
			...
			if bytes.Contains(line, []byte("/Volumes/")) ||
				...
				for _, m := range matches {
					...
					if !strings.Contains(pStr, "/.agyp/") &&
						!strings.Contains(pStr, "\\.agyp\\") &&
...
// IsInternalAutomatedSession checks if a session was generated by an internal automated tool (like agyp commit check).
...
func IsInternalAutomatedSession(userPrompt string) bool {
	...
	if strings.HasPrefix(p, "[AGYP_INTERNAL_COMMIT_CHECK]") ||
		strings.HasPrefix(p, "[AGYP_INTERNAL_TITLE_GEN]") ||
...
func cleanPromptSummary(raw string) string {
	...
	if strings.HasPrefix(raw, "[AGYP_INTERNAL_") {
		return "(No prompt summary)"
	}
